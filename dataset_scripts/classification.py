...
def load_file(path, mode, is_attack = 1, label = 1, folder_name='Bi/', sliceno = 0, verbose = True):
    ...
    if os.path.getsize(path)//10 ** 9 > 0: #cvs is grater than 0GB or not
        x = np.zeros((0,0))
        for chunk in pd.read_csv(path, chunksize=10 ** 6): #divid into 10^6 size
            chunk.drop(columns = columns_to_drop_packet, inplace = True)# deleteing data from cvs [ columns_to_drop_packet]
            chunk = chunk[chunk.columns.drop(list(chunk.filter(regex='mqtt')))] #those who  lines contains [mqtt]in coloum will eleminates
                                     
            chunk = chunk.fillna(-1) # filles missing values to (-1)
